...
public interface Typeof<Term, Ty, Bind> extends GTermAlg<Term, ITypeof<Ty, Bind>>,
	...
	@Override
	default ITypeof<Ty, Bind> TmTimesFloat(Term t1, Term t2) {
		return ctx -> {
	...
	@Override
	default ITypeof<Ty, Bind> TmFloat(float p1) {
		return ctx -> tyAlg().TyFloat();
	}

	@Override
	default ITypeof<Ty, Bind> TmString(String p1) {
		return ctx -> tyAlg().TyString();
	}

	@Override
	default ITypeof<Ty, Bind> TmLet(String x, Term t1, Term t2) {
		return ctx -> {
			Ty tyT1 = visitTerm(t1).typeof(ctx);
