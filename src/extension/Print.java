...
public interface Print<Term, Bind> extends GTermAlg<Term, IPrint<Bind>>, varapp.Print<Term, Bind>,
	...
	@Override
	default IPrint<Bind> TmString(String s) {
		return ctx -> s;
	}

	@Override
	default IPrint<Bind> TmFloat(float f) {
		return ctx -> String.valueOf(f);
	}

	@Override
	default IPrint<Bind> TmTimesFloat(Term t1, Term t2) {
		return ctx -> "timesfloat " + visitTerm(t1).print(ctx) + " " + visitTerm(t2).print(ctx);
	}

	@Override
	default IPrint<Bind> TmLet(String x, Term t1, Term t2) {
