...
public interface TyEqv<Ty, Bind, Term> extends utils.TyEqv<Ty, Bind> {
	@Override
	TyEqual<Ty, Bind> tyEqual();
	SimplifyTy<Ty, Bind, Term> simplifyTy();

	@Override
	default boolean tyEqv(Context<Bind> ctx, Ty ty1, Ty ty2) {
		Ty tyS1 = simplifyTy().visitTy(ty1).apply(ctx);
		Ty tyS2 = simplifyTy().visitTy(ty2).apply(ctx);
		return tyEqual().visitTy(tyS1).apply(tyS2);
	}

	interface TyEqual<Ty, Bind>
			extends TyAlgQuery<Ty, Function<Ty, Boolean>>, simplebool.TyEqv.TyEqual<Ty>, tyarith.TyEqv.TyEqual<Ty> {
		@Override
		TyAlgMatcher<Ty, Boolean> matcher();

		@Override
		default Function<Ty, Boolean> TyVar(int x, int n) {
			return ty -> matcher().TyVar(y -> m -> x == y).otherwise(() -> false).visitTy(ty);
		}
